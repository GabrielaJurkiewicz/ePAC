...
function [] = plotContinuousSummaryAllFP(EEG,fP,fA,dirOut,ID,alpha,methodName,plotWithMask,plotWithoutMask)
    ...
    for epoch  = 1:EEG.trials
        if strcmp(methodName,'eMI')
            if sum(strcmp(ll,['v' ID '_' methodName '_epoch' num2str(epoch) '_results.mat']))>0
                ...
                if num == 0
                    MIxtime = zeros(EEG.trials,length(fA),length(fP));
                    MIpvalxtime = NaN(EEG.trials,length(fA),length(fP));
                    MIStatxtime = zeros(EEG.trials,length(fA),length(fP));
                    Origins     = zeros(EEG.trials,length(fA),length(fP));
                end
                if plotWithoutMask
                    MIxtime(epoch,:,:) = comodulogram;
                end
                if plotWithMask
                    MIpvalxtime(epoch,:,:) = pval;
                    MIStatxtime(epoch,:,:) = comodulogramStat;
                    Origins(epoch,:,:) = couplingOrigins;
                end
                num = 1;
            end
        else
    ...
    if plotWithMask
        ...
        if strcmp(methodName,'eMI')
            ...
            afterFDRMin = min(afterFDR,[],3);
            afterFDR = max(afterFDR,[],3);
            afterFDR(afterFDR==0) = afterFDRMin(afterFDR==0);
        else
            afterFDR = max(afterFDR,[],3);
        end
        afterFDR = afterFDR';
    ...
    Titles = {};
    if plotWithMask
        Titles = [Titles 'AllFreq_Stat'];
    end
    if plotWithoutMask
        Titles = [Titles 'AllFreq'];
    end
    for i = 1:length(Titles)
        ...
        print(f, '-dpng', '-r300', [directoryOut title '.png']);
        close(f);
    end
end
